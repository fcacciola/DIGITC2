...
public sealed class ResultCatalogService {
  ...
  private static double? TryReadScore(string resultFile) {
    ...
    var lScoreLine = File.ReadLines(resultFile).Where(line => line.StartsWith("Score:", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
    if ( string.IsNullOrEmpty(lScoreLine) ) 
      return null ;

    var rawScore = lScoreLine["Score:".Length..].Trim();
